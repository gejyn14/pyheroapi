...
class Account:
    ...
    def balance(self) -> Dict[str, Any]:
        ...
        try:
            raw_balance = self._get_cached_or_fetch('balance', lambda: self._client.get_account_balance(self.account_number))
            return {
                'account_number': self.account_number,
                'total_balance': float(raw_balance.total_balance) if raw_balance.total_balance else 0.0,
                'available_balance': float(raw_balance.available_balance) if raw_balance.available_balance else 0.0,
                'securities_balance': float(raw_balance.securities_balance) if raw_balance.securities_balance else 0.0,
                'timestamp': datetime.now().isoformat()
            }
        except Exception as e:
            logger.error(f"Failed to get balance for account {self.account_number}: {e}")
            return {
                'account_number': self.account_number,
                'error': str(e),
                'total_balance': 0.0,
    ...
    def positions(self) -> List[Dict[str, Any]]:
        ...
        try:
            raw_positions = self._get_cached_or_fetch('positions', lambda: self._client.get_positions(self.account_number))
            return [
                {
                    'symbol': pos.symbol,
                    'quantity': int(pos.quantity) if pos.quantity else 0,
                    'average_price': float(pos.average_price) if pos.average_price else 0.0,
                    'current_price': float(pos.current_price) if pos.current_price else 0.0,
                    'market_value': float(pos.market_value) if pos.market_value else 0.0,
                    'unrealized_pnl': float(pos.unrealized_pnl) if pos.unrealized_pnl else 0.0,
                    'unrealized_pnl_rate': float(pos.unrealized_pnl_rate) if pos.unrealized_pnl_rate else 0.0,
                    'timestamp': datetime.now().isoformat()
                }
                for pos in raw_positions
            ]
        except Exception as e:
            logger.error(f"Failed to get positions for account {self.account_number}: {e}")
            return []
    ...
    def get_profit_loss(self, symbol: str, start_date: str, end_date: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get profit/loss for a specific stock."""
        try:
            if end_date:
                raw_pnl = self._client.get_period_stock_profit_loss(symbol, start_date, end_date)
            else:
            ...
            return [
                {
                    ...
                    'commission': float(pnl.tdy_trde_cmsn) if pnl.tdy_trde_cmsn else 0.0,
...
class KiwoomAPI:
    ...
    def connect(
        ...
        try:
            ...
            logger.info(f"✅ Connected to Kiwoom API ({'sandbox' if sandbox else 'production'})")
            return cls(client)
        except Exception as e:
