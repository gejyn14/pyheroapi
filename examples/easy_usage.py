...
def main():
    ...
    with pyheroapi.connect(app_key, secret_key, sandbox=True) as api:
        ...
        print(
            f"Samsung Current Price: ₩{price:,}"
            if price
            else "Samsung: Price unavailable"
        )
        ...
        if "error" not in quote:
            print(
                f"Best Bid: ₩{quote['best_bid']:,}"
                if quote["best_bid"]
                else "Best Bid: N/A"
            )
            print(
                f"Best Ask: ₩{quote['best_ask']:,}"
                if quote["best_ask"]
                else "Best Ask: N/A"
            )
        ...
        if "error" not in etf_info:
            ...
            print(
                f"Tracking Error: {etf_info['tracking_error']}%"
                if etf_info["tracking_error"]
                else "Tracking Error: N/A"
            )
        ...
        if "error" not in elw_info:
            ...
            print(
                f"Strike Price: ₩{elw_info['strike_price']:,}"
                if elw_info["strike_price"]
                else "Strike: N/A"
            )
        ...
        if "error" not in balance:
            print(
                f"Total Balance: ₩{balance['total_balance']:,}"
    ...
    try:
        ...
        print(
            f"Samsung price: ₩{samsung_price:,}"
            if samsung_price
            else "Samsung price: N/A"
        )
