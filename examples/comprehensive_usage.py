...
def main():
    ...
    with pyheroapi.connect(APP_KEY, SECRET_KEY, sandbox=True) as api:
        ...
        for pos in positions[:5]:  # Show first 5
            ...
            print(
                f"    P&L: ₩{pos['unrealized_pnl']:,.0f} ({pos['unrealized_pnl_rate']:.2f}%)"
            )
        ...
        for order in unfilled[:3]:  # Show first 3
            print(
                f"  #{order['order_number']}: {order['symbol']} {order['order_quantity']:,} @ ₩{order['order_price']:,.0f}"
            )
        ...
        for order in filled[:3]:  # Show first 3
            print(
                f"  #{order['order_number']}: {order['symbol']} {order['filled_quantity']:,} @ ₩{order['filled_price']:,.0f}"
            )
        ...
        print(
            f"  Best Bid: ₩{quote['best_bid']:,.0f} (Qty: {quote['total_bid_quantity']:,})"
        )
        print(
            f"  Best Ask: ₩{quote['best_ask']:,.0f} (Qty: {quote['total_ask_quantity']:,})"
        )
        ...
        try:
            ...
            for pnl in pnl_data:
                ...
                print(
                    f"  Trade: {pnl['quantity']:,} shares @ ₩{pnl['sell_price']:,.0f}"
                )
        ...
        if buy_result["success"]:
            ...
            for order in unfilled:
                if order["order_number"] == buy_result["order_number"]:
                    print(
                        f"   Found our order: {order['remaining_quantity']} shares remaining"
                    )
                    ...
                    if modify_result["success"]:
                        ...
                        print(
                            f"   New Order Number: {modify_result['new_order_number']}"
                        )
                    ...
                    if cancel_result["success"]:
                        ...
                        print(
                            f"   Cancelled Order Number: {cancel_result['cancelled_order_number']}"
                        )
