...
from pyheroapi import KiwoomAPIError, KiwoomClient
...
def analyze_etf(client: KiwoomClient, symbol: str, name: str):
    ...
    try:
        ...
        periods = {"0": "1 Week", "1": "1 Month", "2": "6 Months", "3": "1 Year"}
        ...
        for period_code, period_name in periods.items():
            try:
                ...
                if returns:
                    ...
                    print(
                        f"  {period_name}: ETF {etf_return}% | Benchmark {benchmark_return}%"
                    )
...
def compare_etfs(client: KiwoomClient, etf_symbols: List[str]):
    ...
    if etf_data:
        print(
            f"{'Symbol':<10} {'Name':<20} {'NAV':<12} {'Track Err':<10} {'1Y Return':<10}"
        )
        print("-" * 70)
        for etf in etf_data:
            print(
                f"{etf['symbol']:<10} {etf['name'][:19]:<20} {etf['nav']:<12} "
